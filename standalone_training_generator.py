#!/usr/bin/env python3
"""
Standalone training pair generation system for ARC-AGI challenges.
Generates thousands of training pairs from 2x2 to 24x24 grids.
"""
...
class StandaloneTrainingGenerator:
    """Standalone generator with sophisticated transformation rules."""
    ...
    def generate_random_grid(self, rows: int, cols: int, 
                           density: float = 0.3,
                           color_distribution: Optional[Dict[int, float]] = None) -> List[List[int]]:
    ...
    def generate_structured_grid(self, rows: int, cols: int, 
                               structure_type: str) -> List[List[int]]:
        """Generate a grid with specific structural patterns."""
        grid = [[0 for _ in range(cols)] for _ in range(rows)]
        
        if structure_type == "rectangles":
            # Add random rectangles
            max_rects = max(1, min(3, rows * cols // 20))
            num_rects = random.randint(1, max_rects)
            for _ in range(num_rects):
                ...
                if rows > 1 and cols > 1:
                    ...
                    # For very small grids, just fill a single cell
                    grid[0][0] = color
        
        elif structure_type == "lines":
            # Add horizontal and vertical lines
            max_lines = max(1, min(4, max(rows, cols) // 2))
            num_lines = random.randint(1, max_lines)
            for _ in range(num_lines):
                color = random.randint(1, 9)
                if random.choice([True, False]) and cols > 1:  # Horizontal line
                    row = random.randint(0, rows-1)
                    start_col = random.randint(0, cols-2)
                    end_col = random.randint(start_col+1, cols-1)
                    for j in range(start_col, end_col+1):
                        grid[row][j] = color
                elif rows > 1:  # Vertical line
    ...
    def apply_color_operation(self, grid: List[List[int]], rule_type: str, 
                            complexity: ComplexityLevel) -> List[List[int]]:
    ...
    def apply_shape_operation(self, grid: List[List[int]], rule_type: str, 
                            complexity: ComplexityLevel) -> List[List[int]]:
    ...
    def apply_pattern_recognition(self, grid: List[List[int]], rule_type: str, 
                                complexity: ComplexityLevel) -> List[List[int]]:
    ...
    def generate_math_grid(self, rows: int, cols: int) -> List[List[int]]:
    ...
    def apply_mathematical_operation(self, grid: List[List[int]], rule_type: str, 
                                   complexity: ComplexityLevel) -> List[List[int]]:
    ...
    def generate_geometric_grid(self, rows: int, cols: int) -> List[List[int]]:
        """Generate a grid with geometric features."""
        grid = [[0 for _ in range(cols)] for _ in range(rows)]
        
        # Add geometric points
        max_points = max(1, min(5, rows * cols // 8))
    ...
    def apply_geometric_operation(self, grid: List[List[int]], rule_type: str, 
                                complexity: ComplexityLevel) -> List[List[int]]:
...
def main():
    ...
    generator = StandaloneTrainingGenerator(config)
    challenges = generator.generate_all_challenges()
    
    # Save challenges
    output_path = Path("/workspace/large_training_challenges.json")
